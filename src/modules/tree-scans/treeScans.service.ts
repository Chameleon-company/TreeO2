...
const ensureProjectExists = async (projectId: number) => {
    ...
    if (!project) {
        throw new AppError(404, TREE_SCAN_MESSAGES.PROJECT_NOT_FOUND, ERROR_CODES.DATA_001);
    }

    if (!project.isActive) {
        throw new AppError(400, TREE_SCAN_MESSAGES.PROJECT_INACTIVE, ERROR_CODES.VAL_002);
    }
...
const ensureSpeciesAssignedToProject = async (
    ...
    if (!species) {
        throw new AppError(404, TREE_SCAN_MESSAGES.SPECIES_NOT_FOUND, ERROR_CODES.DATA_001);
    }
...
const ensureScanExists = async (id: number) => {
    ...
    if (!scan) {
        throw new AppError(404, TREE_SCAN_MESSAGES.SCAN_NOT_FOUND, ERROR_CODES.DATA_001);
    }
...
export class TreeScansService {
    ...
    async createTreeScan(data: CreateTreeScanInput) {
        ...
        await ensureUserExists(data.farmerId, TREE_SCAN_MESSAGES.FARMER_NOT_FOUND);
    ...
    async updateTreeScan(id: number, data: UpdateTreeScanInput, changedBy: number) {
        try {
        ...
        if (
            ...
            await ensureUserExists(nextFarmerId, TREE_SCAN_MESSAGES.FARMER_NOT_FOUND);
            await ensureUserExists(
            nextInspectorId,
            TREE_SCAN_MESSAGES.INSPECTOR_NOT_FOUND,
            );
    ...
    async recycleFob(fobId: string) {
        if (!fobId.trim()) {
        throw new AppError(400, TREE_SCAN_MESSAGES.FOB_ID_REQUIRED, ERROR_CODES.VAL_003);
        }
