/*
  ...
  - `change_type` is added to `tree_scan_audit` as a required column (V1.2 7.21).
    Because a NOT NULL column with no default aborts on a populated table, it is
    added nullable, backfilled, then set NOT NULL. Existing audit rows are all
    produced by the tree scan correction flow, so they are backfilled as
    'corrected'.
...
-- enforce NOT NULL.
ALTER TABLE "tree_scan_audit" ADD COLUMN     "change_type" VARCHAR(50);
