...
describe("Tree Types API", () => {
	...
	const createTreeType = async (
		...
		const treeType = await prisma.treeType.create({
			data: {
				...
				key: overrides.key === undefined ? nextUnique("key") : overrides.key,
				scientificName:
	...
	const createTreeScanReference = async (
		speciesId: number,
		projectId: number,
	) => {
			...
			await prisma.projectTreeType.deleteMany({
				where: {
					OR: [
						...
						{ projectId: { in: number[] } } | { treeTypeId: { in: number[] } }
					>,
				},
			});
		}
