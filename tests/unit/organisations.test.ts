...
describe("OrganisationsService - Unit Tests", () => {
	...
	describe("updateOrganisation", () => {
		...
		it("should throw 409 when updating a deactivated organisation", async () => {
			mockedOrganisation.findUnique.mockResolvedValue(
				buildOrganisation({ accountActive: false }),
			);

			await expect(
				organisationsService.updateOrganisation(1, { name: "Test" }),
			).rejects.toThrow(AppError);
